...
page rank algorithm\cite{page1999pagerank}, and there are many useful
...
$\mathbb{R}^{|V|}$, where
\begin{equation}
\mathbf{x}=\alpha DA \mathbf{x} + \beta\mathbf{1},
\end{equation}
$A$ is the adjacency matrix of $G_t$, and $D$ is a diagonal matrix that
\begin{equation}
D_{ij}=\begin{cases}
0&(i\neq j)\\
...
The equivalent form of $x(G_t,v_i)$ is 
\begin{equation}
x_i(G_t,v_i)=\alpha\sum_jA_{ij}\mathit{Act}(G_t,v_j)x_j(G_t,v_j)+\beta.
\end{equation}
...
\begin{itemize}
\item \textsc{CurrentTime}, a non-negative real number.
\item \textsc{EdgeList}[$1\dots|V|$][$1\dots\textsc{EdgeListLen}$], an array with fixed length, every of
whose entry points to a sorted array, consisting of the out edges of $v_i$
in the graph, with elements 
\begin{itemize}
