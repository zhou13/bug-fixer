...
\item $\mu$, a real number in range $(0,1)$.
...
adds $e(v_i,v_j,t,w)$ to the graph and update the graph,
defined in Algorithm \ref{alg-add}.
\item \textsc{Query Degree}($t,v$)

returns $k(G_t,v)$, defined in Algorithm \ref{alg-deg}.
\item \textsc{Query Connectivity}($t,u,v$)

returns whether $u$ and $v$ is connected in the network at time $t$.
\item \textsc{Query Connectivity}($t,u,v$)

returns whether $u$ and $v$ is connected in the network at time $t$.
\item \textsc{Query Activity}($t,v$)

returns the evolving activity $a_{\mu,\eta}(G_t,v)$.
\item \textsc{Query Centrality}($t$, $\alpha$, $\beta$, $\mathbf{x}_0$, $m$)

returns the active centrality vector $\mathbf{x}(G_t)$. Algorithm
\ref{alg-cen} use the iteration method start with $\mathbf{x}_0$ and for
$m$ times.
\end{itemize}
...
\EndIf
\State $p\gets \Call{FindRoot}{i}$;
\State $q\gets \Call{FindRoot}{j}$;
\If{$p\neq q$}
...
\begin{algorithm}[htbp]
\caption{\textsc{Find Root}($i$)}\label{alg-find}
\textbf{Time Complexity:} $O(\log |V|)$\\
\textbf{Additional Space:} $O(1)$
\begin{algorithmic}[1]
\Require $1\leq i\leq |V|$
\Ensure return the root of the union tree containing $v_i$.
\State $\mathit{currentTime}\gets t$;
\If {$\mathit{father}[i].u>0$}
...
\begin{center} \begin{tikzpicture}[y=.3cm, x=.4cm, font=\sffamily]
 	%axis
	\draw (0,0) -- coordinate (x axis mid) (31, 0);
	\draw (0,0) -- coordinate (y axis mid) (0, 19);
	%ticks
	\foreach \x in {2,4,...,30}
		\draw (\x,1pt) -- (\x,-3pt) node[anchor=north] {\scriptsize \x0};
	\foreach \y in {3,6,...,18}
		\draw (1pt,\y) -- (-3pt,\y) node[anchor=east] {\scriptsize \y};
	%labels      
	\node[below=0.5cm] at (x axis mid) {\scriptsize Number of Querys ($\times10^3$)};
	\node[rotate=90, left=0.8cm] at (0, 15) {\scriptsize Running Time (sec.)};
	%plots
	\draw plot[mark=*, mark options={fill=white}] file {exp-n10w};
	\draw plot[mark=triangle*, mark options={fill=white}] file {exp-n50w};
	\draw plot[mark=square*, mark options={fill=white}] file {exp-n2w};
