...
is considered to be vertex while friendship is considered to be edge. We
suppose the evolution of a social network overtime only includes bidirectional
new connections between exsisting or newly created nodes, and we assume the
graph will evolve as a typical social network. We study the centrality and
activity metrics for nodes in such graphs by regulate a series of reasonable
properties the metrics should satisfy. We then formally define our metrics and
give their mathematical analysis. We have developped a practical based
framework to maintain the evolving network and the algorithm to compute our
...
\textbf{Keywords.} social network, dynamic graph, centrality, activity, algorithm. 
\end{abstract}
