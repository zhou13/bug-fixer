...
\caption{\textsc{AddEdge}($v_i,v_j,t,w$)}
\label{alg-add}
...
\State append ($j$,$t$,$w$,$z_2$) to \textit{edgeList}[$i$];
\State append ($i$,$t$,$w$,$z_1$) to \textit{edgeList}[$j$];
...
\begin{algorithm}[htbp]
\caption{\textsc{FindRoot}($i$)}\label{alg1}
\textbf{Time Complexity:} $O(\log |V|)$\\
\textbf{Additional Space:} $O(1)$
\begin{algorithmic}[1]
