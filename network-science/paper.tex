...
\begin{algorithmic}[1]
\Require $1\leq i,j\leq |V|$ and $t\geq currentTime$ and $w>0$.
\Ensure $G_t$ is up-to-date.
\State $\mathit{currentTime}\gets t$;
\State append ($j$,$t$,$w$) to \textit{edgeList}[$i$];
\State append ($i$,$t$,$w$) to \textit{edgeList}[$j$];
...
The facility functions of union-find set is defined in follow.
\begin{algorithm}
\caption{\textsc{FindRoot}($i$)}
\textbf{Time Complexity:} $O(\log |V|)$\\
\textbf{Additional Space:} $O(1)$
\begin{algorithmic}[1]
...
\If {$x>y$}
	\State \Return{\Call{FindRoot}{$\mathit{father}[i]$}};
\Else
	\State \Return{$i$};
\EndIf
\end{algorithmic}
